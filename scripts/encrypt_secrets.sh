...
for file in $files; do
  ...
  yaml | yml)
    ...
    if [[ "$kind" == "Secret" || "$kind" == "ConfigMap" ]]; then
      ...
      #sops  --encrypt --input-type yaml --output-type yaml -encrypted-regex '^(data|stringData)$' "$file" > "$new_file"
      sops --config .sops.yaml --encrypt --input-type yaml --output-type yaml "$file" >"$new_file"
    fi
    # Encrypt YAML file with sops and save it to the new file
    sops --encrypt --input-type yaml --output-type yaml "$file" >"$new_file"
    ;;
  json)
    # Encrypt JSON file with sops and save it to the new file
    sops --encrypt --input-type json --output-type json "$file" >"$new_file"
    ;;
  *)
    # Default case: Encrypt file with sops and save it to the new file
    sops --encrypt "$file" >"$new_file"
    ;;
  esac
done
